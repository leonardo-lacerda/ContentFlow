...
export const MediaBox: FC<{
  ...
  const maximize = useCallback(
    (media: Media) => async (e: any) => {
      e.stopPropagation();
      const project = (media as any).carouselProject;
      const carouselChildren = (media as any).isCarousel
        ? (media as any).children || []
        : [];
      modals.openModal({
        title: '',
        top: 10,
  ...
  const deleteImage = useCallback(
    (media: Media) => async (e: any) => {
      ...
      const mediaItems = (media as any).isCarousel
        ? (media as any).children || []
        : [media];
      await Promise.all(
