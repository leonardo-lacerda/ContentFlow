...
const STATUS_COLORS: Record<string, { border: string; bg: string; glow: string }> = {
  DRAFT: { border: 'border-white/10', bg: 'bg-white/[0.02]', glow: '' },
  ANALYZING: { border: 'border-blue-500/30', bg: 'bg-blue-500/5', glow: 'shadow-blue-500/10' },
  NEEDS_REVIEW: { border: 'border-amber-500/30', bg: 'bg-amber-500/5', glow: 'shadow-amber-500/10' },
  ACTIVE: { border: 'border-emerald-500/30', bg: 'bg-emerald-500/5', glow: 'shadow-emerald-500/10' },
  FAILED: { border: 'border-red-500/30', bg: 'bg-red-500/5', glow: 'shadow-red-500/10' },
};
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
              <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                ...
                <div className="text-[24px] font-[800] text-white leading-none">{metrics.total}</div>
                <div className="text-[11px] text-white/40 mt-1">Total de marcas</div>
              </div>
              <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4">
                ...
                <div className="text-[24px] font-[800] text-emerald-400 leading-none">{metrics.active}</div>
                <div className="text-[11px] text-emerald-400/60 mt-1">Ativas com DNA</div>
              </div>
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
                ...
                <div className="text-[24px] font-[800] text-amber-400 leading-none">{metrics.needsReview}</div>
                <div className="text-[11px] text-amber-400/60 mt-1">Precisam revisão</div>
              </div>
              <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4">
                ...
                <div className="text-[24px] font-[800] text-blue-400 leading-none">{metrics.analyzing}</div>
                <div className="text-[11px] text-blue-400/60 mt-1">Sendo analisadas</div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3 mb-4">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2.5 text-[13px] bg-white/[0.04] border border-white/10 rounded-[10px] text-white placeholder:text-white/30 outline-none focus:border-white/25 transition-colors"
                />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {filtered.length === 0 ? (
                <div className="col-span-full text-center py-12 text-white/30 text-[13px]">
                ...
                filtered.map((brand) => {
                  ...
                  return (
                    <div
                      key={brand.id}
                      className={`group relative rounded-2xl border ${colors.border} ${colors.bg} p-5 transition-all hover:shadow-lg ${colors.glow} hover:border-white/20 cursor-pointer`}
                      onClick={() => router.push(`/brands/${brand.id}`)}
                    >
                      {/* Selected badge */}
                      {brand.selected && (
                        <div className="absolute top-3 right-3">
                          <span className="inline-flex items-center gap-1 text-[10px] font-[700] text-emerald-400 bg-emerald-500/15 px-2 py-0.5 rounded-full">
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="text-[15px] font-[700] text-white truncate pr-16">{brand.name}</h3>
                          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                            <BrandStatusBadge status={brand.status} />
                            {brand.industry && (
                              <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/[0.06] border border-white/10 text-white/40">
                                {brand.industry}
                              </span>
                            )}
                          </div>
                          {brand.website && (
                            <div className="text-[11px] text-white/30 mt-2 flex items-center gap-1 truncate">
                      ...
                      <div className="flex items-center gap-2 mt-4 pt-3 border-t border-white/[0.05] opacity-0 group-hover:opacity-100 transition-opacity">
                        {!brand.selected && (
