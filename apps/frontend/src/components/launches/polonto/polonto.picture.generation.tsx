...
const GenerateTab = observer(({ store }: any) => {
  ...
  return (
    <>
      ...
      >
        {t('generate_image_with_ai', 'IA Generate')}
        {data?.credits ? `(${data?.credits} left)` : ``}
      </div>
      <InputGroup
        placeholder="Descreva a imagem que deseja gerar para o ContentFlow..."
        onKeyDown={(e) => {
          if (e.key === 'Enter') {
            handleGenerate();
          }
      ...
      >
        {data?.credits <= 0 ? 'Comprar mais créditos' : 'Gerar imagem'}
      </Button>
      {image && (
        <ImagesGrid
          shadowEnabled={false}
...
export const PictureGeneratorSection = {
  ...
  Tab: (props: any) => (
    <SectionTab name="IA Generate" {...props}>
      <Clean />
    </SectionTab>
  ),
