...
// A logo must never be an invented graphic mark. Every image (single or
// carousel slide) carries this rule so the model renders a brand only as its
// name in type, not as an emblem/icon/monogram it makes up on the spot.
const LOGO_RULE =
  'Logo rule: if any brand logo appears, render it ONLY as the brand name written in clean typography (a plain text wordmark). Never invent or draw an icon, symbol, emblem, monogram, badge, mascot or any graphic mark — text only.';
