...
const IssueSchema = z.object({
  type: z.enum(['warning', 'blocker']).nullable().optional().describe('Issue severity: warning or blocker'),
  slideIndex: z.number().int().min(0).nullable().optional().describe('Slide index where the issue occurs'),
  slide: z.number().int().min(0).nullable().optional().describe('Alias for slideIndex (legacy format)'),
  field: z.string().nullable().optional().describe('The field with the issue (headline, body, cta, etc.)'),
  severity: z.enum(['low', 'medium', 'high']).nullable().optional().describe('Severity level (legacy format)'),
  message: z.string().nullable().optional().describe('Human-readable description of the issue'),
  issue: z.string().nullable().optional().describe('Alias for message (legacy format)'),
  suggestion: z.string().nullable().optional().describe('Suggested fix for the issue'),
});
...
const TemplateCheckResultSchema = z.object({
  ...
  message: z.string().nullable().optional().describe('Detail about the check result'),
  matchedSlide: z.number().int().min(0).nullable().optional().describe('Slide that triggered the check'),
});
