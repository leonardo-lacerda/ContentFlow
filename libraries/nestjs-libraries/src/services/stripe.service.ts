...
export class StripeService {
  ...
  async createOrGetCustomer(organization: Organization) {
    ...
    const customer = await stripe.customers.create({
      email: users.users[0].user.email.indexOf('@') > -1 ? users.users[0].user.email : `${users.users[0].user.email}@contentflow.com`,
  ...
  private async createEmbeddedCheckout(
    ...
    try {
      await stripe.customers.update(customer, {
        email: user.email.indexOf('@') > -1 ? user.email : `${user.email}@contentflow.com`,
        ...(body.dub
          ? {
              metadata: {
