  ...
  private buildAgent(id: string, tools: Record<string, any>) {
    return new Agent({
      ...
      Intent routing (do this first, on every user message):
        ...
        - Never answer a request that maps to a tool with plain prose, a markdown list or a clarifying question you could have answered with a sensible default. Calling the right tool IS the answer.
        - Route as follows:
          ...
          - Asking to CREATE an image, video, carousel or text/copy ("cria um carrossel sobre X", "faz uma imagem de Y", "escreve uma legenda", "monta um reels") -> call creationOptionsTool, then the showCreationOptions action, and wait for the user's confirmed selection before generating.
        ...
        - When the request is genuinely ambiguous, still prefer acting with sensible defaults over interrogating the user. Ask at most one short question, and only when the missing detail actually blocks the result.
