  ...
  async agent() {
    ...
    return new Agent({
      ...
      Creative Studio behavior:
        ...
        - If the user's message contains [--contentflow-intent--], follow that instruction as an internal UI contract: use the matching frontend action instead of falling back to plain prose.
        - If the user's message contains [--content-action--], it is a structured click from the ContentFlow artifact UI. Parse the ACTION and PAYLOAD, preserve the selected idea or carousel, and continue the requested workflow without asking the user to repeat it. A transform-carousel action starts the carousel options-to-copy workflow; it is not a generation confirmation. An approve-carousel-copy action means the user edited the copy; call showCarouselPreview again with the edited slides and do not generate images yet. A generate-image action means showCreationOptions must be rendered before the generation flow. A generate-images action must preserve designSpec and require both copyApproved=true and designApproved=true before generation.
        ...
        - For a carousel, call only showCarouselPreview and include final copy, visualDirection, layout and imagePrompt for every slide so the user sees a horizontal visual preview in the chat. A plain slide-by-slide text outline is never a sufficient carousel result.
