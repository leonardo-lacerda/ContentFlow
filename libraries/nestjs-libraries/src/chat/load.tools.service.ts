  ...
  async agent() {
    ...
    return new Agent({
      ...
      Creative Studio behavior:
        ...
        - If the user's message contains [--content-action--], it is a structured click from the ContentFlow artifact UI. Parse the ACTION and PAYLOAD, preserve the selected idea or carousel, and continue the requested workflow without asking the user to repeat it. A transform-carousel action starts the carousel options-to-copy workflow; it is not a generation confirmation. An approve-carousel-copy action means the user edited the copy; call contentPresentationTool again with the edited slides and do not generate images yet. A generate-image action means showCreationOptions must be rendered before the generation flow. A generate-images action must preserve designSpec and require both copyApproved=true and designApproved=true before generation.
