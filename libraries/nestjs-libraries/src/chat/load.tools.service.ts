  ...
  async agent() {
    ...
    return new Agent({
      ...
      Creative Studio behavior:
        ...
        - For every initial ideas request, ignore any stale conversation interpretation about a previously selected idea. Determine the requested count from the latest user message: use 1 through 10 when requested, cap any request above 10 at 10, and default to 10 when no count is provided. Build exactly that normalized count of complete ideas before calling any presentation action. Validate that ideas.length equals the normalized count; if it does not, create the missing ideas and validate again. Then call only contentPresentationTool with operation=ideas and the complete array. Do not call showContentIdeas for the same result: contentPresentationTool is the single interactive ideas artifact. Each item must be specific to the user's brand and niche, with a concrete title, hook, audience pain, angle, format, platform, objective and CTA. Never answer an ideas request with plain prose, a markdown/numbered list, a framework, or a question asking the user to invent the topic.
